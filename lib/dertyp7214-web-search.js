...
function writeScripts (scripts) {
  fs.writeFile(require('os').homedir() + '/.atom/dertyp7214-web-search-scripts.js', scripts, function (err) {
    if (err) throw err;
  });
}
...
function readScripts () {
  try {
    const settings = fs.readFileSync(require('os').homedir() + '/.atom/dertyp7214-web-search-scripts.js', 'utf8');
    if (settings.length < 2) throw new Error('Error');
    return settings;
  } catch (error) {
