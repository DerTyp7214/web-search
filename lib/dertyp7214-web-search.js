        ...
        for (i = 0, end = json.replacements.length; i < end; i++) {
          ...
          if (filename === obj.file) {
            try {
              ...
              if (obj.path.split('/')[0] !== '') {
                console.log('IF');
                for (y = 0, thisFuckingFirstEnd = obj.path.split('/').length; y < thisFuckingFirstEnd; y++) {
                ...
                let newObj = {};
                for (z = paths.length, thisFuckingSecondEnd = 0; z > thisFuckingSecondEnd; z--) {
                  newObj = JSON.parse('{"' + paths[z - 1] + '": ' + JSON.stringify(Object.keys(newObj).length > 0 ? newObj : object) + '}');
                }
...
function writeScripts (scripts) {
  fs.writeFile(require('os').homedir() + '/.atom/dertyp7214-web-search-scripts.json', scripts, function (err) {
    if (err) throw err;
  });
}
...
function readScripts () {
  try {
    const settings = fs.readFileSync(require('os').homedir() + '/.atom/dertyp7214-web-search-scripts.json', 'utf8');
    if (settings.length < 2) throw new Error('Error');
    return settings;
  } catch (error) {
    writeSettings('let editor;\r\n\r\n// infologs: services & dals\r\nif (editor = atom.workspace.getActiveTextEditor()) {\r\n  editor.selectAll();\r\n  let selection = editor.getSelectedText();\r\n  const regex = /.*: async function \\(.*\\)(| ){/g;\r\n  editor.insertText(selection.replace(regex, (match, g1, g2) => {\r\n    return match + `\\n\\t\\tlog.info(this.${match.split(':')[0].trim()}.name, {${match.split('(')[1].split(')')[0].replace('transaction, ', '')}})`;\r\n  }));\r\n  editor.save();\r\n}\r\n\r\n// erorrlogs: services & dals\r\nif (editor = atom.workspace.getActiveTextEditor()) {\r\n  editor.selectAll();\r\n  let selection = editor.getSelectedText();\r\n  const regex = /.*: async function([\\s\\S](?!async function))*?log\\.error\\(.*\\)/g;\r\n  let count = 0;\r\n  const should = selection.match(/log\\.error\\(.*\\)/g).length;\r\n  editor.insertText(selection.replace(regex, (match, g1, g2) => {\r\n    count++;\r\n    return match.replace(/log\\.error\\(.*\\)/, `log.error(this.${match.split(':')[0].trim()}.name, ${match.split('catch (')[1].split(')')[0]})`);\r\n  }));\r\n  console.log(`%creplaced ${count} of ${should} errorlogs!`, 'background: red; color: white; font-size: 28pt;');\r\n  editor.save();\r\n}\r\n\r\n// errorlogs: controller\r\nif (editor = atom.workspace.getActiveTextEditor()) {\r\n  editor.selectAll();\r\n  let selection = editor.getSelectedText();\r\n  const regex = /async function.*([\\s\\S](?!async function))*?log\\.error\\(.*\\)/g;\r\n  let count = 0;\r\n  const should = selection.match(/log\\.error\\(.*\\)/g).length;\r\n  editor.insertText(selection.replace(regex, (match, g1, g2) => {\r\n    count++;\r\n    return match.replace(/log\\.error\\(.*\\)/, `log.error(${match.split('function')[1].split('(')[0].trim()}.name, ${match.split('catch (')[1].split(')')[0]})`);\r\n  }));\r\n  console.log(`%creplaced ${count} of ${should} errorlogs!`, 'background: red; color: white; font-size: 28pt;');\r\n  editor.save();\r\n}\r\n');
    atom.workspace.open(require('os').homedir() + '/.atom/dertyp7214-web-search-scripts.json', {});
    return '{}';
  }
}
