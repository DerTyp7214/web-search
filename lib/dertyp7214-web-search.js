...
export default {
  ...
  putCustomReplacement (type) {
    const json = JSON.parse(atom.config.get('DerTyp7214-Web-Search')['putCustomReplacement']);
    if (editor = atom.workspace.getActiveTextEditor()) {
      try {
          ...
          if (filename === obj.file) {
            try {
              ...
              for (y = paths.length, end = 0; y > end; y--) {
                newObj[paths[y - 1]] = Object.keys(newObj).length > 0 ? newObj : object;
                for (x = 0, end2 = Object.keys(newObj).length; x < end2; x++) {
                  console.warn(Object.keys(newObj)[x], newObj[Object.keys(newObj)[x]], paths[y - 1]);
                  if (Object.keys(newObj)[x] !== paths[y - 1]) {
                    delete newObj[Object.keys(newObj)[x]];
                  }
                }
              }
              console.log('Finished Object #' + i, newObj);
              // editor.setText(JSON.stringify(object, null, 2));
            } catch (error) {
              console.log(error);
            }
          }
